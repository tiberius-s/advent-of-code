...
export function three(): void {
  ...
  // PART 1
  const binGamma = readings.reduce(countBitsByPosition).map(mapMostCommonVal(readings.length)).join("").toString();
...
// UTILS
function mapMostCommonVal(arrLength: number): (x: number) => number {
  return function (val: number): number {
    return val > arrLength / 2 ? 1 : 0;
  };
}
...
function sumAtIndex(idx: number): (prev: number[], curr: number[]) => number[] {
